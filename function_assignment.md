# Function Assignment by Sub Feature

## Nghĩa - Sign Up & Sign In + Question Management + Bank Operations + Assignment Management (AI Features Lead)

> **Note:** Nghĩa is responsible for all AI-related features. Cooperates with AnhNHH on Assignment Management and Bank Operations.

### Sign Up & Sign In (Iteration 1)
| Function | Effort | Priority |
|----------|--------|----------|
| View Homepage | 1 | Highest |
| Register Account | 3 | Highest |
| Login | 1 | Highest |
| Forgot Password | 2 | High |
| Logout | 1 | Low |
| Change Password | 2 | Medium |

### Assignment Management (Iteration 3)
| Function | Effort | Priority |
|----------|--------|----------|
| View Assignment (Teacher) | 1 | Medium |
| View Assignment (Student) | 1 | High |

### Bank Operations (Iteration 2 & 3)
| Function | Effort | Iteration | Priority |
|----------|--------|-----------|----------|
| Edit Resource Bank | 2 | Iter 2 | Medium |
| View Resource Bank | 2 | Iter 2 | High |
| Reset User Password | 2 | Iter 2 | Medium |
| Delete Resource Bank | 1 | Iter 3 | Low |

### Question Management (Iteration 2)
| Function | Effort | Priority |
|----------|--------|----------|
| Create Question With AI | 4 | High |

### AI Features (Iteration 3)
| Function | Effort | Priority |
|----------|--------|----------|
| Create Assignment With AI | 3 | Highest |
| Grade Essay With AI Feedback | 4 | High |

---

## ToanDV - Submission & Grading + Analytics + Question Management + Assignment Management + Class Participation

### Class Participation (Iteration 1)
| Function | Effort | Priority |
|----------|--------|----------|
| Search Class | 1 | High |
| Join Class | 1 | High |
| Approve/Reject Student Requests | 1 | High |
| View Class Information | 1 | Medium |
| Leave Class | 1 | Low |

### Question Management (Iteration 1)
| Function | Effort | Priority |
|----------|--------|----------|
| Create Question Manual | 2 | High |
| Create Resource Bank | 2 | Highest |

### Submission & Grading (Iteration 3)
| Function | Effort | Priority |
|----------|--------|----------|
| Attempt Assignment | 3 | High |
| Submit Assignment | 2 | High |
| Grade Manual Essay | 2 | Medium |
| View Grade Submissions | 2 | Medium |

### Question Management (Iteration 2)
| Function | Effort | Priority |
|----------|--------|----------|
| Update Question In Resource Bank | 2 | Medium |
| Delete Question In Resource Bank | 1 | Low |
| Download Resource Bank | 2 | Medium |

### Assignment Management (Iteration 2)
| Function | Effort | Priority |
|----------|--------|----------|
| Create Assignment Manual | 3 | Highest |
| Update Assignment | 2 | High |

### Assignment Management (Iteration 3)
| Function | Effort | Priority |
|----------|--------|----------|

### Analytics & Results (Iteration 3)
| Function | Effort | Priority |
|----------|--------|----------|

---

## HuyMT - Class Administration + User & System Management

### Class Administration (Iteration 1)
| Function | Effort | Priority |
|----------|--------|----------|
| Create Class | 2 | Highest |
| Update Class | 2 | High |
| Delete Class | 1 | Low |

### User & System Management (Iteration 1, 2 & 3)
| Function | Effort | Iteration | Priority |
|----------|--------|-----------|----------|
| View User List | 2 | Iter 1 | High |
| View Student Information | 1 | Iter 1 | Medium |
| Remove Student | 1 | Iter 1 | Low |
| Update User Status | 2 | Iter 2 | Medium |
| View System Dashboard | 2 | Iter 2 | Medium |
| View Transaction History | 2 | Iter 2 | Low |

### Analytics & Results (Iteration 3)
| Function | Effort | Priority |
|----------|--------|----------|
| View Grades & Solutions | 2 | Medium |
| View Student Dashboard | 2 | Low |
| View Class Analytics | 3 | Medium |
| Export Grades | 2 | Low |

---

## AnhNHH - Profile Management + Attendance & Scheduling + Assignment Management + Question Management (Cooperates with Nghĩa)

> **Note:** Cooperates with Nghĩa on Assignment Management and Bank Operations.

### Profile Management (Iteration 1)
| Function | Effort | Priority |
|----------|--------|----------|
| Update Profile | 2 | Medium |
| View Profile | 1 | High |
| View Notifications | 1 | Medium |

### Attendance & Scheduling (Iteration 1, 2 & 3)
| Function | Effort | Iteration | Priority |
|----------|--------|-----------|----------|
| Create Class Schedule | 2 | Iter 1 | Highest |
| View Class Schedule (Teacher) | 1 | Iter 1 | High |
| Update Class Schedule | 2 | Iter 1 | High |
| Delete Class Schedule | 1 | Iter 1 | Low |
| View Class Schedule (Student) | 2 | Iter 2 | High |
| View Personal Schedule (Teacher) | 1 | Iter 2 | Medium |
| View Personal Schedule (Student) | 1 | Iter 2 | Medium |
| Take Attendance | 1 | Iter 2 | High |
| Edit Attendance | 1 | Iter 3 | Medium |

### Assignment Management (Iteration 3)
| Function | Effort | Priority |
|----------|--------|----------|
| Create Assignment By Uploading Files | 3 | Highest |
| Create Assignment By Importing From Resource Bank | 2 | High |
| Delete Assignment | 1 | Low |

### Question Management (Iteration 2)
| Function | Effort | Priority |
|----------|--------|----------|
| Create Question By Importing Files | 3 | High |

---

## DuongNC - Learning Materials + Posts & Comments + Virtual Meeting + Subscription

### Learning Class Materials (Iteration 1)
| Function | Effort | Priority |
|----------|--------|----------|
| Upload Class Material | 2 | High |
| View Class Material | 1 | Medium |
| View Materials | 1 | Medium |
| Download Materials | 1 | Low |
| Download Class Material | 1 | Low |
| Delete Class Material | 1 | Low |

### Posts & Comments (Iteration 1)
| Function | Effort | Priority |
|----------|--------|----------|
| Create Post | 2 | Medium |
| Update Post | 1 | Low |

### Posts & Comments (Iteration 2 & 3)
| Function | Effort | Iteration | Priority |
|----------|--------|-----------|----------|
| View Post | 1 | Iter 2 | Medium |
| Create Comment | 1 | Iter 2 | Medium |
| Update Comment | 1 | Iter 2 | Low |
| Delete Post | 1 | Iter 3 | Low |
| View Comment | 1 | Iter 3 | Low |
| Delete Comment | 1 | Iter 3 | Low |

### Virtual Meeting (Iteration 2)
| Function | Effort | Priority |
|----------|--------|----------|
| Join Meeting | 3 | High |

### Subscription Management (Iteration 2)
| Function | Effort | Priority |
|----------|--------|----------|
| Create Subscription | 2 | High |
| Update Subscription | 2 | Medium |

### Subscription Management (Iteration 3)
| Function | Effort | Priority |
|----------|--------|----------|
| View Subscriptions | 1 | Medium |
| View Revenue Dashboard | 2 | Medium |
| Delete Subscription | 1 | Low |
| Buy Subscription | 2 | High |

---

## Summary - Effort per Person per Iteration

| Member | Iteration 1 | Iteration 2 | Iteration 3 | Total |
|--------|-------------|-------------|-------------|-------|
| Nghĩa | 10 | 10 | 10 | 30 |
| ToanDV | 9 | 10 | 9 | 28 |
| HuyMT | 9 | 6 | 9 | 24 |
| AnhNHH | 10 | 10 | 7 | 27 |
| DuongNC | 10 | 10 | 9 | 29 |
| **Total** | **48** | **46** | **44** | **138** |
